    ...
    <main>
        <div class="bg-white py-14 mt-4">
            <div class="mx-auto max-w-7xl px-6 lg:px-8">
                <h1 class="text-[2em] md:text-4xl font-semibold border-t border-gray-200 text-center py-7">{{ $title }}</h1>
                <div
                    ...
                    <article class="flex max-w-2xl flex-col items-start justify-between bg-gray-300 rounded-lg p-5">
                        <div class="flex items-center justify-between gap-x-4 text-xs w-full">
                            <a href="/categories/{{ $post->category->slug }}"
                                class="relative rounded-full bg-{{ $post->category->color }}-400 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100 z-0">{{ $post->category->name }}</a>
